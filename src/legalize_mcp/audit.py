...
Every call to every MCP tool writes one JSON line to:
    ...
    ~/.matematic/audit/legalize-mcp.jsonl

(or to the directory given by ``LEGALIZE_AUDIT_DIR``).

If the write fails, the tool returns an error instead of silently continuing
(Art. 2 CONSTITUTION.md). Inherited verbatim from the eu-legal-mcp connectors.
"""
...
_AUDIT_FILENAME = "legalize-mcp.jsonl"


def _resolve_audit_dir() -> Path:
    env = os.environ.get("LEGALIZE_AUDIT_DIR")
    if env:
