...
SOURCE = 'the legalize-dev law-as-git corpus on GitHub'
...
_FAMILIES: list[dict] = [{'name': 'Multi-jurisdiction legislation corpus', 'tool': 'legalize_list_countries / legalize_search_laws / legalize_get_law / legalize_list_reforms'}]

_GAPS: list[dict] = [{'id': 'LGZ-001', 'family': 'Multi-jurisdiction legislation corpus', 'missing': 'Coverage is whatever the upstream legalize-dev repositories contain; it is a third-party mirror, not an official gazette.', 'fallback': "Confirm against the jurisdiction's official source before relying on a text."}, {'id': 'LGZ-002', 'family': 'Multi-jurisdiction legislation corpus', 'missing': 'There is no fuzzy title lookup - law_id is a filename stem and must come from search or an official citation.', 'fallback': 'Use legalize_search_laws to obtain a law_id.'}, {'id': 'LGZ-003', 'family': 'Multi-jurisdiction legislation corpus', 'missing': 'Search requires a GITHUB_TOKEN; without one, only direct fetches work at lower rate limits.', 'fallback': 'Set GITHUB_TOKEN to enable search.'}, {'id': 'LGZ-004', 'family': 'Multi-jurisdiction legislation corpus', 'missing': "The corpus is NOT MIT-licensed. MIT covers legalize-dev's pipeline; each country "
                       "repository inherits the terms of its own official source - legalize-es follows the BOE "
                       "reuse conditions (citing the source is mandatory), legalize-at is CC BY 4.0, others differ.", 'fallback': 'Read the terms in the country repository before quoting or redistributing a text.'}]
