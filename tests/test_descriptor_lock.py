...
async def _descriptors() -> dict[str, dict]:
    from legalize_mcp.server import mcp

    async with Client(mcp) as client:
        tools = await client.list_tools()
    return {t.name: t.model_dump(mode="json", exclude_none=True) for t in sorted(tools, key=lambda t: t.name)}
...
def test_descriptors_match_lock():
    ...
    changed = sorted(n for n in set(current) & set(locked) if _digest(current[n]) != _digest(locked[n]))
