...
from legalize_mcp.coverage import build_coverage
from legalize_mcp.server import legalize_coverage


def _coverage():
    fn = getattr(legalize_coverage, "fn", legalize_coverage)
...
def test_tool_writes_an_audit_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("LEGALIZE_AUDIT_DIR", str(tmp_path))
    _coverage()
    lines = [
        line
    ...
    assert any("legalize_coverage" in line for line in lines), (
        "an audit entry was written but it does not name legalize_coverage"
    )
